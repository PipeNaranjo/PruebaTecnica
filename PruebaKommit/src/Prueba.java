...
public class Prueba {
	...
	private static String archivo = "src/input2.txt";
		...
		try {
			...
			for (i = 0; i < lineas.size() && !ward; i++) {
				...
				if (nodos[0].equals(nodos[1])) {
					...
					String inicial = lineas.get(i);
					
					ward = hallarCiclo(lineas, inicial, i, inicial);
				}
			...
			if (cadenaInicial[0].equals(cadenaActual[1])) {
				...
				for (int j = 0; j < auxiliar.size() && !resul; j++) {
					String aux = auxiliar.get(j);
						...
						resul = hallarCiclo(lineas, inicial,i+1, aux);
